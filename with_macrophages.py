...
macrophage_positions = [] # Stores (row, col) tuples
macrophage_pool_indices = np.arange(nx * ny)
macrophage_pool_indices = np.setdiff1d(macrophage_pool_indices, infected_indices, assume_unique=True) # Don't place on initially infected
macrophage_chosen_indices = np.random.choice(macrophage_pool_indices, size=num_macrophages, replace=False)
for idx in macrophage_chosen_indices:
    r_idx, c_idx = divmod(idx, nx)
    macrophage_positions.append((r_idx, c_idx))
...
ax1.set_title("ABM (with Macrophages), t=0")
...
def plot_state(t_current):
    ...
    ax1.set_title(f"ABM (with Macrophages), t={t_current:.1f}")
    virus_data_display = V.value.reshape((ny, nx))
    im2.set_data(virus_data_display)
    current_max_virus = np.max(virus_data_display)
    if current_max_virus > 0: # Prevent setting vmax to 0
        im2.set_clim(vmin=0, vmax=max(current_max_virus, 0.1))


    if macrophage_positions:
        ys, xs = zip(*macrophage_positions) # ys are rows, xs are cols
        macrophage_plot.set_data(xs, ys)
    else:
        macrophage_plot.set_data([],[])
    plt.pause(0.01)

def get_neighbors(r_idx, c_idx):
    neighbors = []
...
for step in range(steps):
    ...
    for r_macro, c_macro in macrophage_positions:
        ...
        fipy_idx_current = c_macro + r_macro * nx
        max_val = V[fipy_idx_current]

        for nr, nc in get_neighbors(r_macro, c_macro):
            fipy_idx_neighbor = nc + nr * nx
            if V[fipy_idx_neighbor] > max_val:
                best_r, best_c = nr, nc
                max_val = V[fipy_idx_neighbor]

        fipy_idx_degrade = best_c + best_r * nx
